...
//         final BufferedImage depth = new BufferedImage(width, height, BufferedImage.TYPE_3BYTE_BGR);
        ...
        device.light(LEDStatus.LED_BLINK_GREEN);
        device.light(LEDStatus.LED_YELLOW);
        device.light(LEDStatus.LED_BLINK_YELLOW);
        device.light(LEDStatus.LED_BLINK_RED_YELLOW);
        device.light(LEDStatus.LED_BLINK_GREEN);
...
//                 System.out.println( "isDirect: " + data.isDirect() );
   ...
   private static void loadConfigurationProperties()
   {
      ...
      try
      {
         double clusters = Double.parseDouble( (String)props.get( "kinect.clusters" ) );
