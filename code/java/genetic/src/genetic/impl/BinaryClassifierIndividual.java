 ...
 * 
 */
public class BinaryClassifierIndividual extends Individual
{

   /**
    * Default constructor.
    */
   public BinaryClassifierIndividual()
   {
   }
   ...
   {
      Individual indy = new BinaryClassifierIndividual();
   ...
   public double evaluateFitness()
   {
      ...
      int                 s1      = (int)    ( (double) targets[ 0 ].length * _myGenotype.get( 1 ) );
      int                 s2      = (int)    ( (double) targets[ 0 ].length * _myGenotype.get( 2 ) );
      double              e       = (double) ( (double) targets[ 0 ].length * _myGenotype.get( 3 ) );
      ArrayList< Double > genome  = new ArrayList< Double >( _myGenotype.size() - 4 );

      for( int i=4; i<_myGenotype.size(); i++ )
      {
      ...
      double[][]            streams = new double[ 1 ][ length ];
      double                local   = 0.0;
      double                value   = 0.0;
      int                   left    = 0;
      ...
      for( int i=0; i<targets.length; i++ )
      {
         ...
         for( int j=0; j<length; j++ )
         {
            ...
            local += Math.abs( value - targets[ i ][ s2 + j ] );
         }
   ...
   public String toStringFinal()
   {
      ...
      for( int i=0; i<targets.length; i++ )
      {
         ...
         for( int j=0; j<length; j++ )
         {
            ...
            local  += Math.abs( value - targets[ i ][ s2 + j ] );
            result += "\n  A target: " + targets[ i ][ s2 + j ] + " value: " + value;
         }
